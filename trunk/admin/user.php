	...
	<form id="1" name="form" method="post"  action="">
		<table >
			...
			echo '
				...
				<tr>
					<td><b>Code postale:</b></td><td><input name="zip" type="" value="'.$user1['zip'].'" size="20" /></td>
				<tr>
				<tr>
			...
			if ($rright['admin_user_profile']!='0')
			{
				...
				<tr>
					...
					<td>
						...
						<input type="radio" name="profile" value="3" '; if ($user1['profile']=='3')echo "checked"; echo '> Superviseur <i>(création, visualisation, accès aux statistiques)</i> <br />
			...
			if ($rright['admin_user_view']!='0')
			{
				echo '
				<tr>
					<td><b>Vues personnelles</b><br><i>(associe des catégories à l\'utilisateur)</i></td>
					<td>';
	...
	<table>
		...
		if ($rparameters['user_advanced']!='0')
		{
			echo '
				...
				<tr>
					<td><b>Code postale:</b></td><td><input name="zip" type="" value="'.$user['zip'].'" size="20" /></td>
				<tr>
				<tr>
...
else
{
	...
	//Display Buttons
		...
		echo'<form name="add" method="post" action="index.php?page=admin&subpage=user&action=add"  id="thisform">';
		echo '
	...
	';
					...
					echo "
						...
						";
				$query = mysql_query("SELECT * FROM `tusers` ORDER BY lastname");
				while ($row=mysql_fetch_array($query)) 
				{
					//find profile name
					...
					echo "<tr class=\"blue\">
							<td width=\"75px\">
								<center>
									...
									{echo "<a title=\"Désactiver cliquez pour Activer\" href=\"./index.php?page=admin&amp;subpage=user&amp;id=$row[id]&amp;action=enable\"><img src=\"./images/access_min.png\" border=\"0\" /></a>";}		
					echo "
								</center>
							</td>
