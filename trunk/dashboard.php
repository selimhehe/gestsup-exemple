...
///// SQL QUERY
		...
		*/
		...
		if ($_GET['searchengine']=='1')
		{
			...
			AND	tincidents.techread LIKE '$techread'
		...
		$masterquery = mysql_query("SELECT tincidents.* $from ORDER BY $_GET[order] $_GET[way], date_create DESC LIMIT $_GET[cursor],$rparameters[maxline]"); 
?>
<div class="post">
<h2>
...
<span class="description">Nombre: <?php echo $resultcount[0] ?></span>
	...
	<table cellspacing="0" cellpadding="0" >
		...
		{ 
			...
			<tr class=\"$bgcolor\" onclick=\"document.location='./index.php?page=$pageticket&amp;id=$row[id]&amp;state=$_GET[state]&amp;techid=$_GET[techid]'\">
			 ...
			 if ($_SESSION['profile_id']!=0 || $_SESSION['profile_id']!=4 || $_GET['techid']=='%') 
			 {
				echo "<td ><center><a class=\"td\" href=\"./index.php?page=$pageticket&amp;id=$row[id]&amp;state=$_GET[state]&amp;techid=$_GET[techid]\">$resulttech[firstname]</a></center></td>";
			 } 
	...
	if  ($resultcount[0]>$rparameters['maxline'])
	{
		...
		for ($i = 1; $i <= $pagenum; $i++) {
			...
			if($_GET['searchengine']==1)
			{echo "<span class=\"pagination\"> <a href=\"index.php?page=dashboard&amp;techid=$_SESSION[user_id]&amp;state=$_GET[state]&amp;order=$_GET[order]&amp;way=$_GET[way]&amp;cursor=$_GET[cursor]&amp;searchengine=1&amp;keywords=$keywords[0]\">&nbsp;$i&nbsp;</a></span> ";}
			else
			{echo "<span class=\"pagination\"><a href=\"index.php?page=dashboard&amp;techid=$_SESSION[user_id]&amp;state=$_GET[state]&amp;order=$_GET[order]&amp;way=$_GET[way]&amp;cursor=$_GET[cursor]\">&nbsp;$i&nbsp;</a></span> ";}
...
$q = mysql_query("SELECT * FROM `tevents` WHERE technician LIKE '$uid' and disable='0' "); 
while ($event=mysql_fetch_array($q))
{
	$devent=explode(" ",$event['date']);
	if ($devent[0]<=$daydate) 
	{
