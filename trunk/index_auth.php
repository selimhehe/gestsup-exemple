...
if($_GET['state']=='') $_GET['state'] = '%';
	...
	if (isset($_POST['submit']))
	{
		...
		if ($findnom != "") 
		{
			...
			echo '<div id="valide"><img alt="logo" src="./images/valide.png" style="border-style: none" alt="img" /> Vos identifiants sont correctes.</div>';
		...
		else if (($rparameters['ldap'])=='1' && ($rparameters['ldap_auth']=='1'))
		{
			...
			if ($bind = @ldap_bind($ldap, "$login@$domain", $pass)) 
			{
				...
				// if error with login or password 
				echo '<div id="erreur"><img src="./images/access.png" alt="erreur" style="border-style: none" alt="img" /> Votre nom d\'utilisateur ou mot de passe, n\'est pas correcte.</div>';
				$www = "./index.php";
				session_destroy();
				//web redirection to login page
				echo "<SCRIPT LANGUAGE='JavaScript'>
			...
			<div id="sidebar">
				...
				<ul>
					...
					while ($row=mysql_fetch_array($reqstate))
					{
						...
						echo "<li "; if (($_GET['state']==$row['id']) && ($_GET['techid']==$_SESSION['user_id'])) echo "class=\"active\""; echo '><a href="./index.php?page=dashboard&amp;techid='.$_SESSION['user_id'].'&amp;state='.$row['id'].'">'.$row['name'].' ('.$cnt[0].')</a></li>';
					}
					echo "
				</ul>";
					...
					if ($row[0]!='')
					{
						...
						<ul>';
							...
							while ($row=mysql_fetch_array($query))
							{
								...
								//count entries
								$q= mysql_query("SELECT COUNT(*) FROM `tincidents` WHERE category='$row[category]' AND subcat LIKE'$subcat' AND (state='1' OR state='2') AND disable='0'");
				...
				if ($rright['side_all']!=0)
				{
					...
					<ul>
						...
						echo "<li "; if (($_GET['techread']=='0') && ($_GET['techid']!=$_SESSION['user_id']) && ($_GET['techid']!=$_SESSION['user_id'])) echo "class=\"active\""; echo '><a href="./index.php?page=dashboard&amp;techid=%&amp;techread=0">Non lu ('.$cnt4[0].')</a></li>';
						...
						while ($row=mysql_fetch_array($reqstate))
						{
							...
							$cnt= mysql_fetch_array($cnt);
							echo "<li "; if (($_GET['state']==$row['id']) && ($_GET['techid']!=$_SESSION['user_id'])) echo "class=\"active\""; echo '><a href="./index.php?page=dashboard&amp;techid=%&amp;state='.$row['id'].'">'.$row['name'].' ('.$cnt[0].')</a></li>';
						}
						echo "
					</ul>";
				}
