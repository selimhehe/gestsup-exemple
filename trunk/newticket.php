...
<div id="catalogue">
	<form name="thisform" enctype="multipart/form-data" method="post" action="" id="thisform">
		...
		<select class="textfield" id="user" name="user" onchange="submit();">
			...
			$row = mysql_fetch_array($query);
			if ($userreg=="") {echo "<option selected value=\"$_POST[user]\">$row[firstname] $row[lastname]</option>";}
			?>
		</select>
		...
		<select class="textfield" id="technician" name="technician" >
		   	<?php
			$querytech = mysql_query("SELECT * FROM `tusers` WHERE disable='0' and id LIKE '$_SESSION[user_id]' ");
			$row=mysql_fetch_array($querytech);
			echo "<option value=\"$row[id]\" selected >$row[firstname] $row[lastname]</option>"; 
			$query = mysql_query("SELECT * FROM `tusers` WHERE disable='0' and profile LIKE '0' and id!='$_SESSION[user_id]' ORDER BY firstname ASC, lastname ASC");
			while ($row=mysql_fetch_array($query)) {echo "<option value=\"$row[id]\">$row[firstname]  $row[lastname]</option>";} 
			?>
		</select>
		<br />
		...
		<br />&nbsp;&nbsp;<img title="Inserer la date et l'heure du jour" src="./images/date.png" onclick="insertAtCaret('resolution','<?php echo $today; ?>');" /><br />
