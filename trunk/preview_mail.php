...
else
{
	...
	echo "<form name=\"mail\" method=\"post\" action=\"\">";
		...
		echo "<div id=catalogue><b>Copie:</b> $rparameters[mail_cc], 
					...
					";
						$quser = mysql_query("SELECT * FROM `tusers` order by firstname asc");
						while ($row=mysql_fetch_array($quser)) {echo "<option value=\"$row[mail]\">$row[firstname] $row[lastname]</option>";}
						if ($creatorrow['mail']!=$techrow['mail']) echo "<option selected value=\"$techrow[mail]\">$techrow[firstname] $techrow[lastname]</option>"; else echo "<option selected value=\"\"></option>"; 
						echo "
					</select>,
