...
if($_POST['modify']||$_POST['quit']||$_POST['mail']||$_POST['upload']||$save=="1") 
{
	...
	if ($globalrow['technician']=='0')
		{
		$_POST['resolution']="$today: Attribution de l\'incident à $r2[firstname].
$_POST[resolution]";
		} else {
		$_POST['resolution']="$_POST[resolution]
$today: Transfert de l\'incident de $r1[firstname] à $r2[firstname]. ";
		}
	}
...
<div id="catalogue">
	<form name="form" enctype="multipart/form-data" method="post" action="" id="thisform">
		...
		if($alarm) echo '<img title="Alame activée le '.$alarm['date'].'" src="./images/clock2.png" />';
		...
		?>
		<a href="./index.php?page=ticket&id=<?php echo $_GET['id']; ?>&action=delete"><img align="right" style="margin:2px 10px 0px 0px; border-style: none;" alt="img" src="./images/delete_max.png" title="Supprimer ce ticket" /></a>
		...
		<img align="right" style="margin:2px 10px 0px 0px; border-style: none;"  alt="img" src="./images/event.png" title="Créer un rappel pour ce ticket" onClick="window.open('./event_add.php?id=<?php echo $_GET['id']; ?>&technician=<?php echo $_SESSION['user_id']; ?>','useradd','width=340,height=300')"/></a>
		</h2>
		<br />
		<label  for="user">Demandeur:</label>
		<select class="textfield" id="user" name="user" onchange="submit();">
		   	<?php
			$query = mysql_query("SELECT * FROM `tusers` WHERE disable='0' ORDER BY firstname ASC, lastname ASC");
			while ($row=mysql_fetch_array($query)) {echo "<option value=\"$row[id]\">$row[firstname] $row[lastname]</option>";}
			//user selection
			if ($_POST['user'])	{$user=$_POST['user'];}	else {$user=$globalrow['user'];}
			$query = mysql_query("SELECT * FROM tusers WHERE id LIKE $user");
			$row = mysql_fetch_array($query);
			echo "<option selected value=\"$user\">$row[firstname] $row[lastname]</option>";
			?>
		</select>
		...
		<input border="0" src="./images/actualiser.png" type="image" value="submit" align="absmiddle" /> 
	<?php
		...
		<select class="textfield" id="technician" name="technician" >
			...
			$query = mysql_query("SELECT * FROM `tusers` WHERE disable='0' and profile LIKE '0' and id!='$globalrow[technician]' ORDER BY firstname ASC, lastname ASC") ;
			while ($row=mysql_fetch_array($query)) {echo "<option value=\"$row[id]\">$row[firstname] $row[lastname]</option>";} 
			?>
		</select>
		<?php
		...
		<br />
		&nbsp;&nbsp;<img title="Inserer la date et l'heure du jour" src="./images/date.png" onclick="insertAtCaret('resolution','<?php if ($uid!=$globalrow['technician'])  echo "$today $reqfname[firstname]:"; else echo "$today: ";?>');" /><br />
